...
class BuildCommand(distutils.command.build.build):
    ...
    def _configure(self):
        #Wget
        if os.path.isdir('/content/libspn/Build-Script') == False:
            git.Git("/content/libspn").clone("https://github.com/S1mmy/Build-Script.git")
            #subprocess.check_call('ln -s /usr/local/lib/python3.7/dist-packages/tensorflow_core/libtensorflow_framework.so.1 /usr/local/lib/python3.7/dist-packages/tensorflow_core/libtensorflow_framework.so')
            tf_path = '/usr/local/lib/python3.7/dist-packages/tensorflow_core/include/tensorflow/core'
            copy_tree('/content/libspn/Build-Script/kernels', tf_path+'/kernels')
            copy_tree('/content/libspn/Build-Script/user_ops', tf_path+'/user_ops')
            copy_tree('/content/libspn/Build-Script/util', tf_path+'/util')
            print("All copied!")
...
class CleanCommand(distutils.command.clean.clean):
    ...
    def _clean(self, files):
        ...
        if os.path.isdir('/content/libspn/Build-Script'):
            shutil.rmtree('/content/libspn/Build-Script')
        for f in files:
            if os.path.exists(f):
                print("Removing " + f)
                os.remove(f)
